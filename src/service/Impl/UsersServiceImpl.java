...
public class UsersServiceImpl implements UsersService{
    ...
    public String getUserByJson(String user, long lastPhotoId, int count, String cookieUser) {
        ...
        try{
            ...
            if (lastPhotoId == -1){
                User m = Factory.getInstance().getUserDAO().getUserByLogin(user);
                int number = Factory.getInstance().getAdvertisementDAO().getAllAdvertisement(m).size();
    ...
    public Long getUserAvatarId(String login) {
        try{
            Long avatar = Factory.getInstance().getUserDAO().getUserByLogin(login).getAvatar();
            return avatar;
